...
@section('content')
    ...
    <div class="bg-white p-6 rounded-lg shadow-lg">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-2xl font-bold">Riwayat Penghasilan</h2>
                    ...
                    @foreach ($penghasilan as $item)
                        <tr class="border-b hover:bg-gray-100 transition duration-150">
                            ...
                            <td class="px-4 py-2 text-sm md:text-base">Rp
                                {{ number_format($item->total_penghasilan, 0, ',', '.') }}</td>
                            ...
                            <td class="px-4 py-2 flex space-x-2">
                                ...
                                <button onclick="hapusPenghasilan({{ $item->id }})"
                                    class="bg-red-500 text-white px-3 py-1 rounded-md hover:bg-red-600 text-sm">
                                    Hapus
                                </button>
                                <form id="delete-form-{{ $item->id }}"
                                    action="{{ route('penghasilan.destroy', $item->id) }}" method="POST"
                                    style="display: none;">
                                    @csrf
                                    @method('DELETE')
                                </form>
